...
st.set_page_config(page_title="Database Design Patterns", layout="wide")

st.markdown(STREAMLIT_STYLING, unsafe_allow_html=True)

with connect_to_weaviate() as client:
    st.markdown(
        "<div class='stHeader'><h1>Database Design Patterns</h1></div>",
    ...
    with col2:
        if len(top_companies) > 0:
            top_companies_str = ", ".join(
                [f"**{k}** ({v})" for k, v in top_companies.items()]
            )
